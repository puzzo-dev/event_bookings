...
def on_customer_before_insert(doc, method=None):
	...
	(``selling/doctype/quotation/mapper.py::_make_customer``).  If the site makes
	customer_group or territory mandatory — commonly via a Property Setter — and
	the Lead supplies neither, that insert raises MandatoryError and ERPNext
	surfaces a blocking "Mandatory Missing" dialog telling the user to go create
	the Customer by hand.  That dialog is the lead-conversion popup we want gone:
	the conversion is supposed to be silent.
