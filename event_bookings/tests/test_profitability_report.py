...
_MODULE = "event_bookings.report.event_booking_profitability.event_booking_profitability"
...
class TestGetData(unittest.TestCase):
	...
	def test_revenue_type_actual(self, mock_frappe, _mock_cogs, _mock_damages):
		mock_frappe.get_all.return_value = [self._make_booking(total_actual=60000)]
		data = get_data({})
		self.assertEqual(data[0]["revenue_type"], "Actual")

	def test_revenue_type_estimated(self, mock_frappe, _mock_cogs, _mock_damages):
		mock_frappe.get_all.return_value = [self._make_booking(total_actual=0, total_estimated=40000)]
		data = get_data({})
