...
class TestSendUnstaffedAlerts(unittest.TestCase):
	def test_sends_alert_for_shortfall(self, mock_frappe, _mock_mgr):
	...
	def test_skips_when_no_managers(self, mock_frappe, _mock_mgr):
		_mock_mgr.return_value = []
		send_unstaffed_alerts()
		mock_frappe.enqueue.assert_not_called()

	def test_handles_no_shortfalls(self, mock_frappe, _mock_mgr):
		mock_frappe.db.sql.return_value = []
		send_unstaffed_alerts()
		mock_frappe.enqueue.assert_not_called()
...
class TestDaily(unittest.TestCase):
	def test_calls_all_subtasks(self, mock_frappe, mock_sync, mock_remind, mock_alert, mock_notify):
		settings = SimpleNamespace(pre_event_reminder_days=3)
