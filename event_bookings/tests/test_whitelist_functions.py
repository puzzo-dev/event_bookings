...
class TestGetItemsFromQuotation(unittest.TestCase):
	def _make_item(self, **kwargs):
		# get_all returns plain dicts, not child-row objects.
		defaults = dict(
			item_code="ITEM-001",
			item_name="Test Item",
			qty=2,
	...
	def test_returns_items_when_permitted(self, mock_frappe, _):
		...
		mock_frappe.get_all.return_value = [self._make_item()]

		result = get_items_from_quotation("QTN-001")

		mock_frappe.has_permission.assert_called_once_with("Quotation", "read", "QTN-001")
		self.assertEqual(mock_frappe.get_all.call_args[0][0], "Quotation Item")
	...
	def test_maps_all_expected_fields(self, mock_frappe, _):
		...
		mock_frappe.get_all.return_value = [
			self._make_item(item_code="X", item_name="Y", qty=3, uom="Box", rate=10.0, amount=30.0)
		]
	...
	def test_returns_multiple_items(self, mock_frappe, _):
		mock_frappe.has_permission.return_value = True
		mock_frappe.get_all.return_value = [
			self._make_item(), self._make_item(item_code="ITEM-002")
		]
...
class TestGetItemsFromSalesOrder(unittest.TestCase):
	...
	def test_returns_items_when_permitted(self, mock_frappe, _):
		mock_frappe.has_permission.return_value = True
		mock_frappe.get_all.return_value = [self._make_item(item_code="SO-ITEM", qty=5)]

		result = get_items_from_sales_order("SO-001")

		mock_frappe.has_permission.assert_called_once_with("Sales Order", "read", "SO-001")
		self.assertEqual(mock_frappe.get_all.call_args[0][0], "Sales Order Item")
		self.assertEqual(len(result), 1)
	...
	def test_maps_all_expected_fields(self, mock_frappe, _):
		mock_frappe.has_permission.return_value = True
		mock_frappe.get_all.return_value = [
			self._make_item(item_code="A", item_name="B", qty=2, uom="Pcs", rate=50.0, amount=100.0)
		]
